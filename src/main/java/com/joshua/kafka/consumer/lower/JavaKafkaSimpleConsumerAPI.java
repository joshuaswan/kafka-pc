...
public class JavaKafkaSimpleConsumerAPI {
    ...
    private Map<KafkaTopicPartitionInfo,List<KafkaBrokerInfo>>
            replicaBrokers = new HashMap<KafkaTopicPartitionInfo,List<KafkaBrokerInfo>>();
     ...
     * @throws Exception
     */
    public void run(long maxReads,KafkaTopicPartitionInfo topicPartitionInfo,
                    List<KafkaBrokerInfo> seedBrokers) throws Exception{
        //默认消费数据的偏移量是当前分区的最早偏移量值
        long whichTime = (long) kafka.api.OffsetRequest.EarliestTime();

        //构建client name 及 groupId
        ...
        try {
            ...
            while (ever || maxReads > 0 ){
                ...
                //如果返回对象表示存在异常，进行异常处理，并进行consumer重新连接操作
                //当异常连接出现次数超过5次的时候，程序抛出异常
                if (response.hasError()) {
                    ...
                    if (code == ErrorMapping.OffsetOutOfRangeCode()){
                        ...
                        readOffSet = this.getLastOffSet(consumer,groupId,topic,partitionID, (Long) kafka.api.OffsetRequest.LatestTime(),clientName);
                        continue;
                    }
                    consumer.close();
                    consumer = null;
                ...
                for (MessageAndOffset messageAndOffset : response.messageSet(topic,partitionID)){
                    ...
                    if (currentOffset < readOffSet){
                        System.out.println("Found and old offset: " + currentOffset + " Exception: " + readOffSet);
                        continue;
                    }

                    //获取下一个读取数据开始的偏移量
                    readOffSet = (long) messageAndOffset.nextOffset();
     ...
     * @return              正常清空下，返回非负数，当出现异常的时候，返回-1
     */
    private long getLastOffSet(SimpleConsumer consumer, String groupId, String topic, int partitionID, long whichTime, String clientName) {
        // 1.从ZK中获取偏移量，当zk的返回偏移量大于0的时候，表示一个正常的偏移量
    ...
    /**
     * 验证分区元数据，如果验证失败，直接抛出IrregularArgumentException
     *
     * @param metadata
     */
     ...
     * 获取方式：
     *
     * @param seedBrokers   Kafka集群连接参数，eg：{"hadoop-senior01" -> 9092,"hadoop-senior02" -> 9092}
     * @param topic         topic名称
     * @param partitionID   分区id
     * @return
     */
    private PartitionMetadata findLeader(List<KafkaBrokerInfo> seedBrokers, String topic, int partitionID) {
        PartitionMetadata returnMetadata = null;

        for (KafkaBrokerInfo brokerInfo : seedBrokers){
            SimpleConsumer consumer = null;

            try {
                // 1.创建简单的消费者连接对象
                consumer = new SimpleConsumer(brokerInfo.brokerHost,brokerInfo.brokerPort,100000,64*1024,"leaderLookUp");
                ...
                System.out.println("Error communicating with Broker [" + brokerInfo.brokerHost + "] to find Leader for [" + topic + ", " + partitionID + "] Reason:" + e);
            }finally {
                if (consumer != null) {
                    try {
                        consumer.close();
